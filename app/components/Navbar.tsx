  ...
  return (
    ...
    >
      <div id="logo" className="flex justify-start items-center gap-[1rem]">
        ...
        <h1
          id="title"
          className="hidden 2xlc:block font-display-space font-semibold text-[1.6rem]"
        >
          Suryarghya Saha
        </h1>
      </div>
      ...
      {linksOpen && (
        <div className="absolute left-0 top-0 2xlc:hidden w-full px-[2rem] pt-[1rem] pb-[3rem] bg-gradient-to-br from-slate-50 via-slate-50 to-slate-50/80 z-50">
          <div
            onClick={handleClick}
            className="2xlc:hidden text-slate-600 flex justify-end cursor-pointer"
          >
            <CgClose size={40} />
          </div>
      ...
      <div id="navlinks">
        {!linksOpen && (
          <div onClick={handleClick} className="2xlc:hidden cursor-pointer">
            <RxHamburgerMenu size={40} />
          </div>
        )}

        <ul className="hidden 2xlc:flex justify-end gap-[2rem] text-[1.2rem] font-medium">
